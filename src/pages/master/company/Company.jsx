...
const Company = () => {
        ...
        const fetchData = async () => {
          const datalist = await Promise.all(
            res.data?.map(async (item) => {
              const file = await getFile(item?.Logo);
  ...
  const handledSaveChange = async (newItem) => {
    ...
    newItem.Logo = newItem.Logo?.Name;
    newItem.InputBy = userId;
  
    try {
      const res = await APIService.Post("Company/Post", newItem);
      if (res.status !== 200) return;
  
      // ดึงไฟล์โลโก้มาใส่
      const file = await getFile(newItem.Logo);
      res.data.File = file;
  ...
  const handledUpdateChange = async (newItem) => {
    ...
    try {
      const res = await APIService.Put("Company/Put", newItem);
      if (res.status !== 200) return;
  ...
  const handledDeleteItem = () => {
    ...
    APIService.Post("Company/Delete", credential)
      .then((res) => {
        if (res.status !== 200) return;
        //remove array
