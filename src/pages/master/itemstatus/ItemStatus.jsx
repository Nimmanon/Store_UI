...
const ItemStatus = () => {
    ...
    const columnTable = [
...

        {
            ...
            format: "",
            action: [
                { event: "edit", display: false },
                { event: "delete", display: false },
            ],
        },
    ];
    ...
    const handledSaveChange = (newItem) => {
        if (exists) return;
        APIService.Post("Status/Post", newItem)
            .then((res) => {
    ...
    const handledUpdateChange = (newItem) => {
        if (exists) return;
        APIService.Put("Status/Put", newItem)
            .then((res) => {
                if (res.status !== 200) return;
                //remove old array
    ...
    const handledDeleteItem = () => {
        ...
        APIService.Post("Status/Delete", credential)
            .then((res) => {
                if (res.status !== 200) return;
                //remove array
                let items = dataList.filter((item) => item.Id !== content.Id);
